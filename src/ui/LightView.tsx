...
export function LightView() {
  return (
    <div>
      ...
      <div className="flame-big lantern-color"><Icon name="lantern" size={52} /></div>
      ...
      {light.txs.map((t, i) => (
        <div key={i} className="tx"><span>+{t.delta} <Icon name="lantern" size={13} /></span> {t.reason}<small>{new Date(t.ts).toLocaleTimeString("zh-CN")}</small></div>
      ))}
    </div>
  );
}
